"""Render ear surface comparison. Usage: python review_ear_surface.py BODY.glb REGISTRATION.json OUTPUT.png"""
...
for id in ['TE17','TE21','SI19','GB2']:
 
 for label,pos,color in [('before',data['earSurfaceReferences']['previousPositions'][id],'#c2410c'),('new',p[id]['position'],'#2563eb')]:
  x,y=xy(pos);d.ellipse((x-4,y-4,x+4,y+4),fill=color);d.text((x+6,y),id+' '+label,fill=color)
d.rectangle((0,0,999,40),fill='white');d.text((20,15),'Retained mesh - lateral view, front is left; orange = previous, blue = revised surface reference',fill='black')
im.save(output)

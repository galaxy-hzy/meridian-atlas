"""Render pinned lip surface evidence; developer inspection, not a clinical figure.
Usage: python review_lip_landmarks.py BODY.glb REGISTRATION.json OUTPUT.png
...
faces=v[t];faces=faces[(faces[:,:,1].max(axis=1)>1.60)&(faces[:,:,1].min(axis=1)<1.685)&(faces[:,:,2].min(axis=1)>.13)]
...
def front(p):return 340+p[0]*7200,810-(p[1]-1.60)*8000
def side(p):return 770+(p[2]-.16)*17000,810-(p[1]-1.60)*8000
...
for id,record in data['lipLandmarks']['points'].items():
 for project in [front,side]:
 ...
 d.text((730,50+30*list(data['lipLandmarks']['points']).index(id)),id+': OBJ/GLB vertex '+str(record['glbVertex']),fill='black')
d.rectangle((0,0,699,54),fill='white');d.text((20,18),'Retained mesh: lip/chin surface review',fill='black')
d.text((30,825),'Orange: previous marker | Green: reviewed surface vertex | Blue: updated marker (3 mm display offset)',fill='black')
d.text((30,850),'GV27 upper-lip tubercle; CV24 midline mentolabial depression. Other face points remain under review.',fill='black')
im.save(output)
